...
/**
 * 2nd generation of IDM (Insert Delete Modify) components
 *
 * based on XML descriptions 
 *
 * IDEA!!
 * cuando dibujo el formulario puedo obtener el ID y hacer un INSERT Dummy
 * tipo 
 *    LOCK table 
 *    INSERT INTO table SET id=''; 
 *    SELECT LAST_INSERT_ID();   // o bien App->getNextId() - 1 (con mayor abstraccion
 *    UNLOCK table
 *
 * y luego cuando escribo a la BD
 * 
 *  UPDATE table .... WHERE id='$id'
 */

class IDM2 extends Component{ 
  /* var: form_file
  ...
  */
  ...
  function publicInit() {
    ...
    $rst    = $_SESSION['_BifApplication']->execQuery("show tables like '$this->table'") ;
    $cant   = $rst->numRows();
    if (DB::isError($cant)) {
      echo (__FILE__ . ' line <b>'. __LINE__ .'</b>: ' .
  ...
  function setInsertView() {
	...
	list($name,$SQLtype,$BIFtype,$BIFparams,$validate,$transform) = $subarray;
      ...
      $this->render_string .= "<FTSubmit value=\"Enviar Datos\" /></FT></TitleBox>";
    }
  ...
  function publicModifyView() {
	...
	$hiddens.="<formhidden name=\"$key\" value=\"".$row[$key]."\" />";
      }
  ...
  function publicInsert() {
	...
	$this->_message = "'". implode(', ',$tmp)  ."' agregado.";
      } else {
